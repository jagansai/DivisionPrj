...
public class DivisionDriver {
    ...
    private static DivisionType DIVISION_TYPE = null;

    public static void setDivisionType( DivisionType divisionType ) {
     ...
     */
    public static Map<String, IDivision> getDivisions(String fileName) throws IOException {
        try {
            return Files.readAllLines(Paths.get(fileName))
                    .stream()
                    .map(x -> x.split(COMMA))
                    .map(x -> createDivision( x[0], Arrays.asList(x[1].split(PIPE_REGEX))))
     ...
     */
    private static IDivision createDivision(String name, List<String> depts) {
        return switch (DIVISION_TYPE) {
     ...
     */
    public static void updateDivisions(Map<String, IDivision> masterDivision,
